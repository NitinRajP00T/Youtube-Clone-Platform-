...
const ProfileMenu = () => {
  ...
  console.log("userID by auth in profile menu", userId);
  ...
  const MenuItem = ({
    ...
    const content = (
      <div className="flex items-center gap-4 px-4 py-2 hover:bg-black/5 dark:hover:bg-yt-hover transition-colors rounded-lg mx-2 cursor-pointer">
        ...
        <span className="text-[14px] text-gray-800 dark:text-yt-text flex-1">
          {label}
        </span>
        {ExtraIcon && (
          <ExtraIcon
            size={16}
            className="text-gray-500 dark:text-yt-text-second"
          />
        )}
      </div>
    );
        ...
        <div className="absolute right-0 top-full mt-2 w-[300px] bg-white dark:bg-[#282828] rounded-xl shadow-[0_4px_32px_0_rgba(0,0,0,0.1)] dark:shadow-[0_4px_32px_0_rgba(0,0,0,0.6)] py-2 border border-black/10 dark:border-yt-border/40 z-50 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex px-4 py-2 mb-2">
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              <span className="text-[16px] font-medium text-gray-900 dark:text-yt-text truncate">
                {user?.username}
              </span>
              <span className="text-[14px] text-gray-600 dark:text-yt-text-second truncate mb-2">
              ...
              {/* <Link 
                            ...
                            >
                             <button onClick={handleViewYourChannel}>View your channel</button>   
                            </Link> */}
              <div
                to={
                  user?.channel ? `/channel/${user.channel}` : "/channel/create"
                }
                onClick={() => setIsOpen(false)}
                className="text-[#065fd4] dark:text-[#3ea6ff] text-[14px] font-medium hover:underline inline-block w-fit"
              >
                <button onClick={handleViewYourChannel}>
                  View your channel
                </button>
              </div>
            </div>
