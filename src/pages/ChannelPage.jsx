...
const ChannelPage = () => {
  ...
  useEffect(() => {
    const fetchChannel = async () => {
      try {
        ...
        setVideos(
          vidRes.data.data.videos.filter((v) => v.channel?._id === channelId),
        );
      } catch (error) {
    ...
    return (
      <div className="text-white max-w-lg mx-auto pt-20 px-4">
        ...
        <form
          ...
          className="flex flex-col gap-4 bg-[#212121] p-6 rounded-xl border border-yt-border shadow-xl"
        >
          <input
            name="channelName"
            placeholder="Channel Name *"
            required
            className="bg-[#121212] border border-yt-border p-3 rounded-md focus:border-blue-500 outline-none transition-colors"
          />
          <textarea
            name="description"
          ...
          <button
            type="submit"
            className="bg-white text-black font-medium py-3 rounded-full mt-2 hover:bg-gray-200 transition-colors"
          >
            Create Channel
          </button>
        </form>
  ...
  if (!channel)
    return (
      <div className="text-white text-center mt-20">Channel not found</div>
    );
  ...
  return (
    <div className="text-white w-full max-w-[1280px] mx-auto pb-20">
      ...
      <div className="border-b border-yt-border px-8 flex gap-8">
        <div className="pb-3 border-b-2 border-white font-medium cursor-pointer">
          Videos
        </div>
        <div className="pb-3 text-yt-text-second font-medium cursor-pointer hover:text-white transition-colors">
          Playlists
        </div>
        <div className="pb-3 text-yt-text-second font-medium cursor-pointer hover:text-white transition-colors">
          Community
      ...
      <div className="px-4 md:px-8 mt-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-4 gap-y-8">
          ...
          {videos.length === 0 && (
            <div className="col-span-full py-10 text-yt-text-second text-center text-sm">
