...
const VideoPage = () => {
    ...
    useEffect(() => {
        const fetchVideoData = async () => {
            try {
                ...
                setSuggestedVideos(allVidRes.data.data.videos.filter(v => v._id !== videoId));
    ...
    const handleLike = async () => {
        ...
        try {
            ...
            setVideo({ ...video, likesCount: res.data.data.likesCount, dislikesCount: res.data.data.dislikesCount });
        } catch (error) {
            console.error(error);
        }
    };
        ...
        return (
            <div className="text-white max-w-[1700px] mx-auto pt-6 px-4 lg:px-8 flex flex-col lg:flex-row gap-6 pb-20">
                <div className="flex-1 w-full lg:max-w-[calc(100%-400px)]">
                    {/* Player Skeleton */}
                    <Skeleton variant="rectangular" width="100%" className="aspect-video" sx={{ bgcolor: 'grey.800', borderRadius: '12px' }} />
                    
                    {/* Title Skeleton */}
                    <Skeleton variant="text" width="70%" sx={{ fontSize: '1.5rem', bgcolor: 'grey.800', mt: 2 }} />
                    <Skeleton variant="text" width="40%" sx={{ fontSize: '1.5rem', bgcolor: 'grey.800' }} />
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-3 gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <Skeleton variant="text" width={120} sx={{ fontSize: '1rem', bgcolor: 'grey.800' }} />
                                <Skeleton variant="text" width={80} sx={{ fontSize: '0.75rem', bgcolor: 'grey.800' }} />
                            </div>
                            <Skeleton variant="rectangular" width={90} height={36} sx={{ bgcolor: 'grey.800', borderRadius: '9999px', ml: 2 }} />
                        </div>
                        <div className="flex items-center">
                            <Skeleton variant="rectangular" width={120} height={36} sx={{ bgcolor: 'grey.800', borderRadius: '9999px' }} />
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col gap-3">
                        {[...Array(6)].map((_, idx) => (
                            <div key={idx} className="flex gap-2 p-2">
                                <Skeleton variant="rectangular" width={160} className="aspect-video flex-shrink-0" sx={{ bgcolor: 'grey.800', borderRadius: '8px' }} />
                                <div className="flex flex-col w-full">
                                    <Skeleton variant="text" width="90%" sx={{ fontSize: '0.875rem', bgcolor: 'grey.800' }} />
                                    <Skeleton variant="text" width="60%" sx={{ fontSize: '0.875rem', bgcolor: 'grey.800', mt: 1 }} />
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        <div className="text-white max-w-[1700px] mx-auto pt-6 px-4 lg:px-8 flex flex-col lg:flex-row gap-6 pb-20">
            <div className="flex-1 w-full lg:max-w-[calc(100%-400px)]">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-3 gap-4">
                    <div className="flex items-center gap-4">
                        <Link to={`/channel/${video.channel?._id}`} className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-bold text-base tracking-tight hover:text-blue-400 transition-colors leading-tight">{video.channel?.channelName || 'Unknown'}</h3>
                                <p className="text-xs text-yt-text-second">{video.channel?.subscribers || 0} subscribers</p>
                            </div>
                        </Link>
                    ...
                    <div className="flex items-center bg-yt-hover rounded-full">
                        <button onClick={handleLike} className="flex items-center gap-2 px-4 py-2 hover:bg-yt-border rounded-l-full border-r border-yt-border transition-colors group">
                        ...
                        <button onClick={handleDislike} className="flex items-center gap-2 px-4 py-2 hover:bg-yt-border rounded-r-full transition-colors group">
                             <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mt-1 group-hover:translate-y-0.5 transition-transform"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zm7-13h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"></path></svg>
                        </button>
                    </div>
                </div>
                ...
                <div className="bg-yt-hover hover:bg-[#3f3f3f]/40 transition-colors rounded-xl p-3 mt-4 text-sm cursor-pointer">
                    <p className="font-medium">{video.views} views • {dayjs(video.createdAt).format('MMM D, YYYY')}</p>
                    <p className="mt-2 text-yt-text whitespace-pre-wrap">{video.description}</p>
                </div>
                ...
                <div className="mt-6">
                    ...
                    {isAuthenticated ? (
                            ...
                            <div className="flex-1 flex flex-col items-end">
                                ...
                                {newComment && (
                                    <div className="flex gap-2 mt-3">
                                        <button type="button" onClick={() => setNewComment('')} className="px-4 py-2 text-sm font-medium rounded-full hover:bg-yt-hover transition-colors">Cancel</button>
                                        <button type="submit" className="px-4 py-2 text-sm font-medium rounded-full bg-blue-500 text-black hover:bg-blue-400 transition-colors">Comment</button>
                                    </div>
                                )}
                            </div>
                        </form>
                        ...
                        <div className="mb-8 p-4 bg-yt-hover rounded-xl text-center">
                            ...
                            <Link to="/auth" className="text-blue-400 hover:text-blue-500">Sign In</Link>
                        </div>
                    )}
                            ...
                            <div key={comment._id} className="flex gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-baseline gap-2">
                                        ...
                                        <span className="text-xs text-yt-text-second">{dayjs(comment.createdAt).fromNow()}</span>
                                    </div>
                                    ...
                                    <div className="flex gap-4 text-yt-text-second items-center text-xs font-medium cursor-pointer">
                                        ...
                                        <span className="hover:text-white transition-colors ml-2">Reply</span>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <div className="w-full lg:w-[400px] flex-shrink-0">
                <div className="flex flex-col gap-3">
                    {suggestedVideos.map(vid => (
                        <Link key={vid._id} to={`/video/${vid._id}`} className="flex gap-2 group cursor-pointer hover:bg-yt-hover p-2 rounded-lg transition-colors -mx-2">
